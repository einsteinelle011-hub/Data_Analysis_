...
# 新增 Message 模型以支持多轮对话的逐条消息存储和检索
# 请不要手动删除此模型，若要移除请同时移除相关服务与迁移
class Message(models.Model):
    ...
    is_user = models.BooleanField(
        default=True
    )  # 标记消息的发送者，True为用户，False为ai
    content = models.TextField()
    timestamp = models.DateTimeField(auto_now_add=True)

    class Meta:
        # 为session和timestamp建立索引
        indexes = [models.Index(fields=["session", "timestamp"])]

    def __str__(self):
        role = "用户" if self.is_user else "AI"
        return f"{role}@{self.session.session_id}: {self.content[:50]}"
